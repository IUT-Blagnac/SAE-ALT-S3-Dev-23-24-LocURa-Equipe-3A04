...
5+|
^|ID ^|Action ^|Comportement attendu ^|Pré-conditions ^|Résultat
^|HTML.2 ^| Se rendre sur la page index.html ^| La page s'affiche avec le style CSS sur tous les éléments ^| Avoir une connexion internet et avoir Docker pour se connecter à la base de données ^|NOK